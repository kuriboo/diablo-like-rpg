/**
 * CharacterMonsters.js - モンスターキャラクター描画機能
 * 
...
import { drawEllipse } from './AnimationPlaceholders';
 ...
 */
export function drawGhostCharacter(graphics, color, width, height, variant, details, direction, animation, customFeatures = {}) {
  // デフォルトのカスタマイズオプション
  const defaults = {
    transparency: 0.7,
    eyeColor: 0x000000,
 ...
 * ウィスプタイプの幽霊を描画
 */
export function drawWispGhost(graphics, color, width, height, centerX, centerY, bodyWidth, bodyHeight, features, animOffsetY, details) {
  // 本体（光の球体）
 ...
 * ファントムタイプの幽霊を描画
 */
export function drawPhantomGhost(graphics, color, width, height, centerX, centerY, bodyWidth, bodyHeight, features, animOffsetY, details, direction) {
  // 残像を先に描画
  if (features.hasTrail) {
    for (let i = 1; i <= features.trailCount; i++) {
 ...
 * シャドウタイプの幽霊を描画
 */
export function drawShadowGhost(graphics, color, width, height, centerX, centerY, bodyWidth, bodyHeight, features, animOffsetY, details, direction) {
  // シャドウは暗い色
 ...
 * 通常の幽霊を描画
 */
export function drawNormalGhost(graphics, color, width, height, centerX, centerY, bodyWidth, bodyHeight, features, animOffsetY, details, direction) {
  // 底部の影
 ...
 * 幽霊の体を描画
 */
export function drawGhostBody(graphics, centerX, centerY, bodyWidth, bodyHeight, direction) {
  // 上半身（丸い頭部）
 ...
 * 幽霊の顔を描画
 */
export function drawGhostFace(graphics, centerX, centerY, bodyWidth, bodyHeight, features, details, direction) {
 ...
 * 幽霊の腕を描画
 */
export function drawGhostArms(graphics, centerX, centerY, bodyWidth, bodyHeight, color, transparency, direction) {
