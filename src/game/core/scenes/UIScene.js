// 必要なモジュールを動的にロードするためのユーティリティ
let PhaserModule = null;
let ReactModule = null;
let ReactDOMModule = null;
...
// Reactを動的にロードする関数
async function getReactModule() {
  if (ReactModule) return ReactModule;
...
// ReactDOMを動的にロードする関数
async function getReactDOMModule() {
  if (ReactDOMModule) return ReactDOMModule;
...
// SkillTreeコンポーネントをインポートするユーティリティ関数
async function getSkillTreeComponent() {
  try {
    const SkillTreeModule = await import('../../../components/ui/SkillTree');
    return SkillTreeModule.default;
  } catch (error) {
    console.error('Failed to load SkillTree component:', error);
    throw error;
  }
}
...
export default class UIScene {
  // 静的なシーンインスタンスとモジュール参照を保持
  static instance = null;
  static React = null;
  static ReactDOM = null;
  static SkillTree = null;
  
  /**
   * Phaserシーンのインスタンス化前に非同期で初期化する
   */
  static async initialize() {
    if (UIScene.instance) return UIScene.instance;
    
    // 必要なモジュールをロード
    const Scene = await getSceneClass();
    UIScene.React = await getReactModule();
    UIScene.ReactDOM = await getReactDOMModule();
    try {
      UIScene.SkillTree = await getSkillTreeComponent();
    } catch (error) {
      console.warn('SkillTree component could not be loaded, will try again when needed:', error);
    }
    
    // Sceneを継承した実装クラス
    class UISceneImpl extends Scene {
      constructor() {
        super({ key: SCENES.UI });
      ...
      async showSkillTree(character = null) {
        // 引数がない場合はプレイヤーを使用
        ...
        try {
          ...
          // SkillTreeが存在しない場合は再ロード
          if (!UIScene.SkillTree) {
            UIScene.SkillTree = await getSkillTreeComponent();
          }
          
          // Reactモジュールの確認
          const React = UIScene.React;
          const ReactDOM = UIScene.ReactDOM;
          
          if (!React || !ReactDOM) {
            throw new Error('React or ReactDOM modules not loaded');
          }
          
          // スキルツリーコンポーネントをレンダリング
          ReactDOM.render(
            React.createElement(UIScene.SkillTree, {
              character: character,
      ...
      hideReactUI() {
        if (this.reactUIContainer) {
          ...
          // React要素のアンマウント
          if (this.reactUIRoot && UIScene.ReactDOM) {
            UIScene.ReactDOM.unmountComponentAtNode(this.reactUIRoot);
          }
        }
      }
  ...
  constructor() {
    if (!UIScene.instance) {
      throw new Error('UIScene must be initialized before instantiation. Call UIScene.initialize() first.');
    }
    return new UIScene.instance();
  }
}